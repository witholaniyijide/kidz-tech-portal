<x-app-layout>
    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-cyan-50 to-teal-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto">
            ...
            <form method="POST" action="{{ route('noticeboard.update', $notice->id) }}" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="content" id="content" rows="8" required
                                ...
                                placeholder="Write the full notice content here...">{{ old('content', $notice->content) }}</textarea>
                        ...
                        <div>
                            ...
                            <select name="priority" id="priority" required
                                class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500">
                                <option value="low" {{ old('priority', $notice->priority) == 'low' ? 'selected' : '' }}>🔵 Low - General information</option>
                                <option value="normal" {{ old('priority', $notice->priority) == 'normal' ? 'selected' : '' }}>🟢 Normal - Standard notice</option>
                                <option value="high" {{ old('priority', $notice->priority) == 'high' ? 'selected' : '' }}>🟠 High - Important announcement</option>
                                <option value="urgent" {{ old('priority', $notice->priority) == 'urgent' ? 'selected' : '' }}>🔴 Urgent - Immediate attention required</option>
                            </select>
                        </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="space-y-3">
                            <label class="flex items-center p-3 rounded-lg bg-gray-50 dark:bg-gray-900/50 hover:bg-gray-100 dark:hover:bg-gray-900 cursor-pointer transition-colors">
                                <input type="checkbox" name="visible_to[]" value="director" {{ in_array('director', $visibleTo) ? 'checked' : '' }}
                ...
                <div class="flex items-center justify-between gap-4 pt-4">
                    <div class="flex gap-3">
                        ...
                        @if($notice->status !== 'archived')
                            <button type="button" onclick="archiveNotice()"
            ...
            {{-- Hidden Archive Form --}}
            <form id="archiveForm" method="POST" action="{{ route('noticeboard.archive', $notice->id) }}" class="hidden">
                @csrf
                @method('PATCH')
            </form>
    ...
    <script>
        function archiveNotice() {
            if (confirm('Are you sure you want to archive this notice? It will no longer be visible to users.')) {
