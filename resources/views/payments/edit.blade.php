<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('payments.show', $payment) }}" style="display: inline-block; padding: 10px 20px; background-color: #6B7280; color: white; font-weight: bold; border-radius: 8px; text-decoration: none;">
                ← Back to Details
            </a>
        </div>
    </x-slot>
                ...
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('payments.update', $payment) }}">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" name="amount" id="amount" value="{{ old('amount', $payment->amount) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                </div>

                                <div>
                                    <label for="payment_date" class="block text-sm font-medium text-gray-700">Payment Date *</label>
                                    <input type="date" name="payment_date" id="payment_date" value="{{ old('payment_date', $payment->payment_date->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="payment_type" id="payment_type" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        <option value="tuition" {{ old('payment_type', $payment->payment_type) == 'tuition' ? 'selected' : '' }}>Tuition</option>
                                        <option value="registration" {{ old('payment_type', $payment->payment_type) == 'registration' ? 'selected' : '' }}>Registration</option>
                                        <option value="materials" {{ old('payment_type', $payment->payment_type) == 'materials' ? 'selected' : '' }}>Materials</option>
                                        <option value="event" {{ old('payment_type', $payment->payment_type) == 'event' ? 'selected' : '' }}>Event</option>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="reference_number" id="reference_number" value="{{ old('reference_number', $payment->reference_number) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="year" id="year" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        ...
                                        <option value="2024" {{ old('year', $payment->year) == '2024' ? 'selected' : '' }}>2024</option>
                                        <option value="2025" {{ old('year', $payment->year) == '2025' ? 'selected' : '' }}>2025</option>
                                        <option value="2026" {{ old('year', $payment->year) == '2026' ? 'selected' : '' }}>2026</option>
                                    </select>
                                </div>

                                <div class="md:col-span-2">
                                    <label for="notes" class="block text-sm font-medium text-gray-700">Notes</label>
                                    <textarea name="notes" id="notes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes', $payment->notes) }}</textarea>
                                </div>
                        ...
                        <div class="flex items-center justify-between mt-8 pt-6 border-t border-gray-200">
                            <a href="{{ route('payments.show', $payment) }}" style="display: inline-block; padding: 12px 24px; background-color: #6B7280; color: white; font-weight: bold; border-radius: 8px; text-decoration: none;">
                                Cancel
                            </a>
