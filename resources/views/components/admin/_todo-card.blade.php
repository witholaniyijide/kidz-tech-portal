...
>
    ...
    <div class="space-y-3 mb-6 max-h-64 overflow-y-auto" role="list" aria-label="Task list">
        <template x-for="(todo, index) in todos" :key="index">
            <div class="flex items-start p-3 rounded-lg bg-white dark:bg-slate-800/50 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all duration-200 group" role="listitem">
                ...
                >
                    <div class="text-gray-800 dark:text-gray-200 group-hover:text-[#423A8E] dark:group-hover:text-[#00CCCD] transition-colors text-sm" :class="{ 'line-through opacity-50': todo.completed }" x-text="todo.text"></div>
    ...
    <form @submit.prevent="addTodo" class="space-y-3 mb-4">
        ...
        <input
            type="date"
            x-model="newDate"
            class="w-full px-4 py-2.5 text-sm text-gray-900 bg-white border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-slate-800 dark:text-white focus:ring-2 focus:ring-[#423A8E] focus:border-[#423A8E] focus-visible:ring-2 focus-visible:ring-[#423A8E]"
