...
{{-- Parent Portal Theme: #F5A623 (Primary Orange/Amber) with Accent: #F7B74A (lighter) / #D4910C (darker) --}}
<aside x-data="{
...
class="fixed left-0 top-0 h-screen bg-white dark:bg-slate-900 flex flex-col transition-all duration-300 z-50 shadow-xl border-r border-gray-200 dark:border-slate-700">
    ...
    <div class="p-4 border-b border-gray-200 dark:border-slate-700">
        <div class="flex items-center" :class="collapsed ? 'justify-center' : 'justify-between'">
            ...
            <a href="{{ route('parent.dashboard') }}" class="flex-shrink-0 flex items-center justify-center" x-cloak>
                ...
                {{-- Fallback Logo with Parent Orange --}}
                <div :class="collapsed ? 'w-10 h-10' : 'w-16 h-16'"
                     class="rounded-xl bg-gradient-to-br from-[#F5A623] to-[#F7B74A] items-center justify-center shadow-lg transition-all duration-300 hidden dark:flex">
    ...
    {{-- Navigation Links (8 Menu Items for Parent) --}}
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        ...
        <a href="{{ route('parent.attendance.index') }}"
           class="flex items-center px-3 py-2.5 rounded-xl transition-all duration-200 group
                  {{ request()->routeIs('parent.attendance.*') ? 'bg-gradient-to-r from-[#F5A623] to-[#F7B74A] text-white shadow-lg' : 'text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700/50 hover:text-gray-900 dark:hover:text-white' }}"
        ...
        <a href="{{ route('parent.payments.index') }}"
           class="flex items-center px-3 py-2.5 rounded-xl transition-all duration-200 group
                  {{ request()->routeIs('parent.payments.*') ? 'bg-gradient-to-r from-[#F5A623] to-[#F7B74A] text-white shadow-lg' : 'text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700/50 hover:text-gray-900 dark:hover:text-white' }}"
           :title="collapsed ? 'Payments' : ''">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
            <span x-show="!collapsed" x-transition class="ml-3 font-medium">Payments</span>
        </a>
        ...
        <a href="{{ route('parent.messages.index') }}"
           ...
           :title="collapsed ? 'Messages' : ''">
            ...
            @php
                $unreadMessages = 0; // TODO: Get actual unread count
            @endphp
            @if($unreadMessages > 0)
                <span x-show="!collapsed" class="ml-auto bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">
                    {{ $unreadMessages }}
                </span>
            @endif
        </a>
        ...
        <a href="{{ route('parent.notices.index') }}"
           class="flex items-center px-3 py-2.5 rounded-xl transition-all duration-200 group
                  {{ request()->routeIs('parent.notices.*') ? 'bg-gradient-to-r from-[#F5A623] to-[#F7B74A] text-white shadow-lg' : 'text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700/50 hover:text-gray-900 dark:hover:text-white' }}"
