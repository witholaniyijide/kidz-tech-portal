    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        ...
        <a href="{{ route('manager.tutor-reports.index') }}"
           ...
           :title="collapsed ? 'Tutor Reports' : ''">
            ...
            <span x-show="!collapsed || window.innerWidth < 768" x-transition class="ml-3 font-medium">Tutor Reports</span>
        </a>
        ...
        <a href="{{ route('manager.notices.index') }}"
           ...
           :title="collapsed ? 'Notices' : ''">
            ...
            @if($unreadNoticesCount > 0)
                <span x-show="!collapsed || window.innerWidth < 768" x-transition class="ml-auto px-2 py-0.5 text-xs font-bold rounded-full {{ request()->routeIs('manager.notices.*') ? 'bg-white/20 text-white' : 'bg-red-500 text-white' }}">
