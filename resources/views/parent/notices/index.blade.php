<x-parent-layout title="Notices">
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            @if($types->count() > 0)
                <form method="GET" action="{{ route('parent.notices.index') }}">
        ...
        @if($notices->count() > 0)
            <div class="space-y-4">
                @foreach($notices as $notice)
                    <a href="{{ route('parent.notices.show', $notice) }}"
                       class="block glass-card rounded-2xl p-5 hover-lift transition-all {{ $notice->is_pinned ? 'border-l-4 border-[#F5A623]' : '' }}">
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex items-start space-x-4">
                                <div class="w-12 h-12 rounded-xl {{ $notice->type === 'urgent' ? 'bg-red-100 dark:bg-red-900/30' : ($notice->type === 'warning' ? 'bg-yellow-100 dark:bg-yellow-900/30' : 'bg-gradient-parent') }} flex items-center justify-center flex-shrink-0">
                                ...
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        @endif
                                        @if($notice->type)
                                            <span class="px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300 rounded-full">
                                                {{ ucfirst($notice->type) }}
                                            </span>
                                        @endif
                                    </div>
