...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $title ?? config('app.name', 'Laravel') }} - Manager Portal</title>
                ...
                <header class="sticky top-0 z-40 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between h-14 md:h-16 px-3 md:px-6">
                        ...
                        <div class="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            @else
                                <h1 class="text-base md:text-xl font-bold text-gray-900 dark:text-white truncate">Manager Portal</h1>
                            @endif
                        ...
                        <div class="flex items-center gap-2 md:gap-4">
                                ...
                                <x-slot name="content">
                                    <div class="w-96 max-h-[32rem] overflow-y-auto">
                                        <div class="px-4 py-3 text-sm font-semibold text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600 sticky top-0 flex items-center justify-between">
                                            ...
                                            @if($unreadCount > 0)
                                                <a href="{{ route('manager.notifications.mark-all-read') }}"
                                                   onclick="event.preventDefault(); document.getElementById('manager-mark-all-read-form').submit();"
                                                   class="text-xs text-orange-600 dark:text-orange-400 hover:underline">
                                                    Mark all as read
                                                </a>
                                                <form id="manager-mark-all-read-form" action="{{ route('manager.notifications.mark-all-read') }}" method="POST" class="hidden">
                                                    @csrf
                                                </form>
                                            @endif
                                        </div>
                                        @php
                                            try {
                                                $notifications = \App\Models\ManagerNotification::where('user_id', auth()->id())
                                                    ->where('is_read', false)
                                        ...
                                        @forelse($notifications as $notification)
                                            <div class="px-4 py-4 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 border-b dark:border-gray-600 transition">
                                                <div class="flex items-start justify-between gap-2">
                                                    ...
                                                    <form action="{{ route('manager.notifications.mark-read', $notification->id) }}" method="POST">
                                                        @csrf
                                        ...
                                        @if($notifications->count() > 0)
                                            <div class="px-4 py-3 bg-gray-50 dark:bg-gray-700 border-t dark:border-gray-600 text-center">
                                                <a href="{{ route('manager.notifications.index') }}" class="text-sm text-orange-600 dark:text-orange-400 hover:underline font-medium">
                            ...
                            <div x-data="{ open: false }" class="relative">
                                ...
                                >
                                    <div class="w-8 h-8 md:w-9 md:h-9 rounded-full bg-gradient-to-br from-[#C15F3C] to-[#DA7756] flex items-center justify-center text-white font-bold text-xs md:text-sm shadow-lg flex-shrink-0">
