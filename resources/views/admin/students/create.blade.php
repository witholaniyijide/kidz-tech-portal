                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="coding_experience" rows="2" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-[#423A8E]">{{ old('coding_experience') }}</textarea>
                            </div>
                            {{-- Career Interest --}}
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Career Interest</label>
                                <textarea name="career_interest" rows="2" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-[#423A8E]">{{ old('career_interest') }}</textarea>
                            </div>
                            {{-- Status --}}
                            <div>
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="starting_course_id" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500">
                                    <option value="">Select Starting Course</option>
                                    @if(isset($courses))
                                        @foreach($courses as $course)
                                            <option value="{{ $course->id }}" {{ old('starting_course_id') == $course->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="current_course_id" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500">
                                    ...
                                    @if(isset($courses))
                                        @foreach($courses as $course)
                                            <option value="{{ $course->id }}" {{ old('current_course_id') == $course->id ? 'selected' : '' }}>
                                                Level {{ $course->level }} - {{ $course->name }}
                                            </option>
                                        @endforeach
                                    @endif
                        ...
                        <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-600">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">Select all courses the student has already completed. Non-sequential completion is allowed.</p>
                                ...
                                @foreach($courses as $course)
                                    <label class="flex items-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                        <input type="checkbox"
                                               ...
                                               value="{{ $course->id }}"
                                               {{ in_array($course->id, old('completed_course_ids', [])) ? 'checked' : '' }}
