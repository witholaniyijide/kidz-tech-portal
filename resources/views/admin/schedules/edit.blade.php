    ...
    <div class="min-h-screen bg-gradient-to-br from-[#423A8E]/5 via-[#00CCCD]/5 to-blue-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 py-8 relative overflow-hidden">
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10" x-data="scheduleEditForm()">
            ...
            <form action="{{ route('admin.schedules.update', $schedule) }}" method="POST">
                ...
                {{-- Rescheduled Class Entries --}}
                <div class="bg-white/60 dark:bg-gray-800/60 backdrop-blur-md border border-white/20 rounded-2xl shadow overflow-hidden mb-6">
                    <div class="px-6 py-4 bg-gradient-to-r from-amber-500 to-orange-500 text-white flex justify-between items-center">
                        <div>
                            <h3 class="text-lg font-semibold">Rescheduled Classes</h3>
                            <p class="text-sm text-white/90 mt-1">Classes moved from another date to today</p>
                        </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="hidden md:grid grid-cols-12 gap-3 mb-3 text-sm font-medium text-gray-600 dark:text-gray-400">
                            ...
                            <div class="col-span-3">Class Link</div>
                            <div class="col-span-1"></div>
                        </div>

                        {{-- Rescheduled Entry Rows --}}
                        <template x-for="(entry, index) in rescheduledEntries" :key="'r-' + index">
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-3 mb-4 p-4 bg-amber-50 dark:bg-amber-900/20 border-2 border-amber-200 dark:border-amber-700/50 rounded-xl">
                                {{-- Student --}}
                                <div class="md:col-span-2">
                                    <label class="md:hidden text-xs text-gray-500 mb-1 block">Student</label>
                                ...
                                <div class="md:col-span-1">
                                    <label class="md:hidden text-xs text-gray-500 mb-1 block">Time</label>
                                    <input type="time" :name="'rescheduled_classes[' + index + '][time]'"
                                           x-model="entry.time"
                                           required
                                           class="w-full px-3 py-2 text-sm border border-amber-300 dark:border-amber-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-amber-500">
                                </div>
                        ...
                        {{-- Empty State --}}
                        <div x-show="rescheduledEntries.length === 0" class="text-center py-8 text-gray-500 dark:text-gray-400">
                            <svg class="w-12 h-12 mx-auto mb-3 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/>
                            </svg>
                            <p>No rescheduled classes. Add one if needed.</p>
                        </div>
    ...
    <script>
        function scheduleEditForm() {
            return {
                ...
                rescheduledEntries: @json($schedule->rescheduled_classes ?? []).map(c => ({
                    student_id: String(c.student_id || ''),
                    tutor_id: String(c.tutor_id || ''),
                ...
                addRescheduledEntry() {
                    const lastEntry = this.rescheduledEntries[this.rescheduledEntries.length - 1];
                    let newStartTime = '09:00';
                    let newEndTime = '10:00';
                ...
                onRescheduledStudentChange(index) {
                    ...
                    const selects = document.querySelectorAll('select[name^="rescheduled_classes"][name$="[student_id]"]');
                    const select = selects[index];
                    const option = select?.options[select.selectedIndex];
