                ...
                <form method="POST" action="{{ route('director.notices.update', $notice) }}">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $notice->title) }}" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-[#4F46E5] focus:ring-[#4F46E5]">
                            @error('title') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Content *</label>
                            <textarea name="content" rows="6" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-[#4F46E5] focus:ring-[#4F46E5]">{{ old('content', $notice->content) }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-4">
                                ...
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="visible_to[]" value="{{ $role }}" {{ in_array($role, old('visible_to', $visibleTo)) ? 'checked' : '' }} class="rounded border-gray-300 text-[#4F46E5] shadow-sm focus:ring-[#4F46E5]">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="datetime-local" name="publish_at" value="{{ old('publish_at', $notice->publish_at?->format('Y-m-d\TH:i')) }}" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-[#4F46E5] focus:ring-[#4F46E5]">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Expires At</label>
                                <input type="datetime-local" name="expires_at" value="{{ old('expires_at', $notice->expires_at?->format('Y-m-d\TH:i')) }}" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-[#4F46E5] focus:ring-[#4F46E5]">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end space-x-4 mt-8">
                        ...
                        <button type="submit" class="px-6 py-2 bg-gradient-to-r from-[#4F46E5] to-[#818CF8] text-white rounded-lg hover:from-[#3730A3] hover:to-[#4F46E5] transition-all">Update Notice</button>
                    </div>
                </form>
            </x-ui.glass-card>
        </div>
