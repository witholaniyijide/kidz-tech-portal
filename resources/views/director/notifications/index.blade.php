<x-app-layout>
    ...
    <x-slot name="title">
        {{ __('Notifications - Director Portal') }}
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    @if($unreadCount > 0)
                        <form action="{{ route('director.notifications.mark-all-read') }}" method="POST">
                            @csrf
                            <button type="submit" class="flex items-center space-x-2 px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
            ...
            @if($notifications->count() > 0)
                <div class="space-y-4">
                    @foreach($notifications as $notification)
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 {{ !$notification->is_read ? 'border-l-4 border-l-indigo-500' : '' }}">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="flex items-center space-x-4 mt-3">
                                            @if(isset($notification->meta['link']))
                                                <a href="{{ $notification->meta['link'] }}" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">
                                                    View Details
                                                </a>
                                            @endif
                                            @if(!$notification->is_read)
                                                <form action="{{ route('director.notifications.mark-read', $notification) }}" method="POST" class="inline">
                                                    @csrf
                                            ...
                                            @endif
                                            <form action="{{ route('director.notifications.destroy', $notification) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
