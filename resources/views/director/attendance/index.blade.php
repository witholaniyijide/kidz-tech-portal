<x-app-layout>
    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 py-12 relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <h1 class="text-4xl font-bold text-gray-900 dark:text-white mb-8">Attendance Management</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
                ...
                <div class="bg-white/30 dark:bg-gray-900/30 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-lg">
                    <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">Approved</div>
                    <div class="text-3xl font-bold text-green-600 dark:text-green-400">{{ $stats['approved'] }}</div>
                </div>
                <div class="bg-white/30 dark:bg-gray-900/30 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-lg">
                    <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">Rejected</div>
                ...
                <div class="bg-white/30 dark:bg-gray-900/30 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-lg">
                    ...
                    <div class="text-3xl font-bold text-blue-600 dark:text-blue-400">{{ $stats['this_month'] }}</div>
                </div>
            </div>
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-100/50 dark:bg-gray-800/50">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Tutor</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($records as $record)
                                <tr class="hover:bg-white/10 dark:hover:bg-gray-800/30 transition">
                                    <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $record->class_date }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $record->student->first_name ?? 'N/A' }} {{ $record->student->last_name ?? '' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $record->tutor->first_name ?? 'N/A' }} {{ $record->tutor->last_name ?? '' }}</td>
                                    <td class="px-6 py-4 text-sm">
                                        <span class="px-3 py-1 rounded-full text-xs font-semibold
                                            {{ $record->status === 'approved' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : '' }}
                                            {{ $record->status === 'pending' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400' : '' }}
                                            {{ $record->status === 'rejected' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' : '' }}">
                                            {{ ucfirst($record->status) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-sm">
                                        <a href="{{ route('director.attendance.show', $record) }}" class="text-blue-600 dark:text-blue-400 hover:underline">View</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
