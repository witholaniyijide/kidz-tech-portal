...
<div class="space-y-6">
                                        ...
                                        <div class="flex items-center gap-4 mt-1 text-sm text-slate-500">
                                            ...
                                            @if($attendance->status === 'approved' && $attendance->monthly_position > 0)
                                                <span class="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-medium">
                                                    ...
                                                    {{ $attendance->monthly_position }}{{ $attendance->monthly_position == 1 ? 'st' : ($attendance->monthly_position == 2 ? 'nd' : ($attendance->monthly_position == 3 ? 'rd' : 'th')) }} of {{ $attendance->monthly_total }} this month
                                                </span>
                                            @endif
                                        </div>
                                        @if($attendance->topic)
    ...
    <div class="flex flex-wrap gap-4">
        ...
        <a href="{{ route('tutor.monthly-schedule.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 font-medium rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-all shadow">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
            </svg>
            Monthly Schedule
