                ...
                <template x-for="(project, index) in projects" :key="index">
                    <div class="flex items-start gap-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                        <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <input type="text" x-model="project.link" :name="'projects['+index+'][link]'" placeholder="Project Link or Description (optional)" {{ !$report->canEdit() ? 'disabled' : '' }} class="px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-[#7978E9] disabled:opacity-50">
                        </div>
                        @if($report->canEdit())
