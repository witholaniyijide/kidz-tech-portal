        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8" x-data="{
                ...
                <div
                    ...
                    x-data="{ showAddTodo: false, editTodo: null }"
                >
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Tasks requiring your attention</p>
                        </div>
                        <button @click="showAddTodo = true" class="p-2 rounded-lg bg-[#4F46E5] text-white hover:bg-[#4338CA] transition-colors" title="Add custom to-do">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="space-y-2 max-h-80 overflow-y-auto" role="list" aria-label="Action items">
                        ...
                        {{-- Custom user todos --}}
                        @if(isset($directorTodos) && count($directorTodos) > 0)
                            <div class="pt-2 mt-2 border-t border-gray-200 dark:border-gray-700">
                                <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2 uppercase tracking-wide">My To-Do List</p>
                            </div>
                            @foreach($directorTodos as $customTodo)
                                <div class="flex items-center justify-between p-3 rounded-lg border transition-all duration-200 group
                                            {{ $customTodo->completed
                                               ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800'
                                               : 'bg-white dark:bg-slate-800/50 border-gray-200 dark:border-gray-700' }}">
                                    <div class="flex items-center gap-3 flex-1 min-w-0">
                                        <form action="{{ route('director.todos.toggle', $customTodo) }}" method="POST" class="flex-shrink-0">
                                            @csrf
                                            <button type="submit" class="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0
                                                {{ $customTodo->completed
                                                   ? 'bg-emerald-500'
                                                   : 'border-2 border-' . ($customTodo->priority === 'high' ? 'red' : ($customTodo->priority === 'medium' ? 'amber' : 'gray')) . '-400 bg-' . ($customTodo->priority === 'high' ? 'red' : ($customTodo->priority === 'medium' ? 'amber' : 'gray')) . '-50 dark:bg-' . ($customTodo->priority === 'high' ? 'red' : ($customTodo->priority === 'medium' ? 'amber' : 'gray')) . '-900/30' }}">
                                                @if($customTodo->completed)
                                                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                                    </svg>
                                                @else
                                                    <span class="w-2 h-2 bg-{{ $customTodo->priority === 'high' ? 'red' : ($customTodo->priority === 'medium' ? 'amber' : 'gray') }}-400 rounded-full"></span>
                                                @endif
                                            </button>
                                        </form>
                                        <div class="flex-1 min-w-0">
                                            <span class="text-sm block truncate {{ $customTodo->completed ? 'text-emerald-700 dark:text-emerald-400 line-through' : 'text-gray-800 dark:text-gray-200' }}">
                                                {{ $customTodo->title }}
                                            </span>
                                            @if($customTodo->due_date)
                                                <span class="text-xs text-gray-500 dark:text-gray-400">Due: {{ $customTodo->due_date->format('M j, Y') }}</span>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-1 ml-2">
                                        <form action="{{ route('director.todos.destroy', $customTodo) }}" method="POST" onsubmit="return confirm('Delete this to-do?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-1.5 text-gray-400 hover:text-red-500 transition-colors" title="Delete">
                        ...
                        @if(count($todos) === 0 && (!isset($directorTodos) || count($directorTodos) === 0))
                            <div class="text-center py-8">
                    ...
                    {{-- Add To-Do Modal --}}
                    <div x-show="showAddTodo" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @keydown.escape.window="showAddTodo = false">
                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full mx-4 p-6" @click.outside="showAddTodo = false">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Add To-Do Item</h3>
                                <button @click="showAddTodo = false" class="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                    </svg>
                                </button>
                            </div>
                            <form action="{{ route('director.todos.store') }}" method="POST" class="space-y-4">
                                @csrf
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Title</label>
                                    <input type="text" name="title" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-[#4F46E5]" placeholder="What needs to be done?">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description (optional)</label>
                                    <textarea name="description" rows="2" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-[#4F46E5]" placeholder="Additional details..."></textarea>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Priority</label>
                                        <select name="priority" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-[#4F46E5]">
                                    ...
                                    <div>
                                        ...
                                        <input type="date" name="due_date" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-[#4F46E5]">
                                    </div>
                                </div>
                                <div class="flex justify-end gap-3 pt-2">
                                    <button type="button" @click="showAddTodo = false" class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">Cancel</button>
                                    <button type="submit" class="px-4 py-2 bg-[#4F46E5] text-white rounded-lg hover:bg-[#4338CA] transition-colors">Add To-Do</button>
                                </div>
                            </form>
                        </div>
                    </div>
