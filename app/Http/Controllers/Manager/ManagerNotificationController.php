...
class ManagerNotificationController extends Controller
{
    ...
    /**
     * Display all notifications for the authenticated manager.
     */
    public function index()
    {
        $notifications = ManagerNotification::where('user_id', Auth::id())
            ->orderBy('created_at', 'desc')
            ->paginate(20);

        $unreadCount = ManagerNotification::where('user_id', Auth::id())
            ->where('is_read', false)
            ->count();
    ...
    {
        ManagerNotification::where('user_id', Auth::id())
            ->where('is_read', false)
            ->update(['is_read' => true]);
     ...
     */
    public function destroy(ManagerNotification $notification)
    {
        abort_unless(
            $notification->user_id === Auth::id(),
            403,
    ...
    {
        $count = ManagerNotification::where('user_id', Auth::id())
            ->where('is_read', false)
            ->count();
